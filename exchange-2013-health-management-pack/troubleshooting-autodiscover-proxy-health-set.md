---
title: Autodiscover.Proxy 正常性セットのトラブルシューティング
TOCTitle: Autodiscover.Proxy 正常性セットのトラブルシューティング
ms:assetid: b6a817cf-0b85-4620-adb7-cc3616c11268
ms:mtpsurl: https://technet.microsoft.com/ja-jp/library/ms.exch.scom.autodiscover.proxy(v=EXCHG.150)
ms:contentKeyID: 53181841
ms.date: 01/28/2016
...
Autodiscover.Proxy が正常でないことを示す警告を受け取った場合、それが示すのは、ユーザーが Exchange の自動検出プロセスを使用して、自分のメールボックスを見つけることができなくなる可能性がある問題です。
...
3.  IIS マネージャーを起動し、問題を報告しているサーバーに接続します。MSExchangeAutodiscoverAppPool が CAS 上で実行されていることを確認します。

4.  <strong>アプリケーション プール</strong> をクリックし、シェルから次のコマンドを実行して、**MSExchangeAutoDiscoverAppPool** アプリケーション プールをリサイクルします。
...
[自動検出サービス](https://technet.microsoft.com/ja-jp/library/bb124251\(v=exchg.150\))
