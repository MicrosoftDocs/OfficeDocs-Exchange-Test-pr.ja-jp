---
title: "\"Legal Hold/法的情報保留\" 役割: Exchange 2013 Help"
TOCTitle: "\"Legal Hold/法的情報保留\" 役割"
ms:assetid: c98ce8ca-3477-479a-ad23-a8e6459bc4d0
ms:mtpsurl: https://technet.microsoft.com/ja-jp/library/Dd876939(v=EXCHG.150)
ms:contentKeyID: 49896473
ms.date: 04/24/2018
mtps_version: v=EXCHG.150
ms.translationtype: HT
---

# \"Legal Hold/法的情報保留\" 役割
...
`Legal Hold` 管理役割を使用すると、管理者はメールボックス内のデータが、訴訟に備えて組織内に保持するかどうかを構成できます。
...
受信者スコープのほか、**Enable-Mailbox** コマンドレット (この役割に含まれます) でも、データベース構成スコープを使用してスコープが限定されます。データベース構成スコープによって、コマンドレットで新しいメールボックスを作成する際にその作成先となるデータベースが制御されます。メールボックスの作成先のデータベースは、データベース スコープ内にある必要があります。これは、**Enable-Mailbox** コマンドレットで *Database* パラメーターを使用してデータベースを指定する場合と、メールボックスの自動配布によりデータベースが選択される場合の両方に適用されます。詳細については、「[管理役割スコープについて](understanding-management-role-scopes-exchange-2013-help.md)」を参照してください。
