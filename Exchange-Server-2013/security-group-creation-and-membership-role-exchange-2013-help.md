...
`Security Group Creation and Membership` 管理役割により、管理者が組織内のユニバーサル セキュリティ グループ (USG) とそれらのメンバーシップを作成および管理できるようになります。

組織で役割ベースのアクセス制御 (RBAC) 分割型アクセス許可モデルが保持されており、USG の作成と管理が Exchange を実行しているサーバーを管理するグループと別のグループによって行われている場合は、この役割をそのグループに割り当てます。

組織で Active Directory 分割型アクセス許可が有効になっている場合、この管理役割に対する委任管理役割以外の割り当てはすべて削除されています。Active Directory 分割型アクセス許可が有効になっている場合、Active Directory 管理者のみが Active Directory 管理ツールを使用して、ユーザーやセキュリティ グループなどの新しいセキュリティ プリンシパルを作成できます。

詳細については、「[分割型アクセス許可について](understanding-split-permissions-exchange-2013-help.md)」を参照してください。
