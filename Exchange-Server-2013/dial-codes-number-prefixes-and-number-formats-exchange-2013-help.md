...
Exchange 管理センター (EAC) の <strong>ダイヤル コード</strong> ページで、UM ダイヤル プランに関するアクセス コード、番号プレフィックス、番号形式を構成します。Exchange 管理シェルで **Set-UMDialPlan** コマンドレットを使用して設定を構成することもできます。すべての設定を構成する、設定を構成しない、または一部の設定のみを構成する、のいずれかを選択できます。各設定で制御されるのはアウトダイヤル プロセスの特定の部分のみです。
