...
次の表に、MRM のトラブルシューティングに使用できるイベントの一覧を示します。ログ出力の種類には、次のものがあります。
  ...
  - <strong>LogAlways</strong> のラベルが付けられたイベントは常に個別にログ出力されます。

  - <strong>LogPeriodic</strong> のラベルが付けられたイベントは発生するたびにログ出力されるのではなく、5 分間に 1 回だけログ出力されます。これによって、ログ エントリが過剰に出力されることを防ぎます。
