---
title: "\"Mail Recipient/メール受信者\" 役割: Exchange 2013 Help"
TOCTitle: "\"Mail Recipient/メール受信者\" 役割"
ms:assetid: 7cf1f1c7-4f32-4446-ac04-99a7f9df2059
ms:mtpsurl: https://technet.microsoft.com/ja-jp/library/Dd876911(v=EXCHG.150)
ms:contentKeyID: 49896334
ms.date: 04/24/2018
mtps_version: v=EXCHG.150
ms.translationtype: HT
---

# \"Mail Recipient/メール受信者\" 役割
...
`Mail Recipients` 管理役割を割り当てられた管理者は、組織内の既存のメールボックス、メール ユーザー、およびメール連絡先を管理することができます。この役割によって、これらの受信者を作成することはできません。受信者を作成するには、`Mail Recipient Creation` 役割を使用します。

この役割の種類では、メールが有効なパブリック フォルダーまたは配布グループを管理できません。これらのオブジェクトを管理するには、次の役割を使用します。
...
分割型アクセス許可モデルに基づいて受信者の作成および管理を別々のグループが実行する組織では、受信者の作成を実行するグループに `Mail Recipient Creation` 役割を割り当て、受信者の管理を実行するグループに `Mail Recipients` 役割を割り当てます。詳細については、以下のトピックを参照してください。
...
受信者スコープに加え、この役割に付属している **Connect-Mailbox** および **Enable-Mailbox** コマンドレットについても、データベース構成スコープを使用してスコープが設定されます。データベース構成スコープは、コマンドレットが新しいメールボックスを作成するデータベースを制御します。メールボックスを作成するデータベースは、データベース スコープ内に存在する必要があります。この条件は、いずれかのコマンドレットで *Database* パラメーターを使用してデータベースを指定する場合、または自動メールボックス配布がデータベースを選択できるようにする場合に適用されます。詳細については、「[管理役割スコープについて](understanding-management-role-scopes-exchange-2013-help.md)」を参照してください。
