---
title: 'ユニファイド メッセージングの役割: Exchange 2013 Help'
TOCTitle: ユニファイド メッセージングの役割
ms:assetid: bf24aa04-5843-4cf3-83e7-3a5fa7bd3522
ms:mtpsurl: https://technet.microsoft.com/ja-jp/library/Dd876935(v=EXCHG.150)
ms:contentKeyID: 49896449
ms.date: 04/24/2018
...
`Unified Messaging` の役割を使用すると、管理者は組織内のユニファイド メッセージング (UM) サービスを管理することができます。

この役割では、UM 固有のメールボックスの構成や、UM プロンプトの管理を実行することはできません。UM に特有のメールボックス構成を管理するには、`UM Mailboxes ` 役割に関連付けられた役割を使用します。UM プロンプトを管理するには、`UM Prompts` 役割に関連付けられた役割を使用します。詳細については、次のトピックを参照してください。
