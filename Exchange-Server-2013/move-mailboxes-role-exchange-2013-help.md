---
title: "\"Move Mailboxes/メールボックスの移動\" 役割: Exchange 2013 Help"
TOCTitle: "\"Move Mailboxes/メールボックスの移動\" 役割"
ms:assetid: 46576498-5d73-495b-8925-b8ce39cfa55e
ms:mtpsurl: https://technet.microsoft.com/ja-jp/library/Dd876879(v=EXCHG.150)
ms:contentKeyID: 49896228
ms.date: 04/24/2018
...
管理者は `Move Mailboxes` 管理役割を使用して、組織内のサーバー間、およびローカルの組織と他の組織のサーバー間で、メールボックスを移動できます。
...
受信者の範囲に加え、この役割に含まれている **New-MoveRequest** コマンドレットもまた、データベース構成の範囲を使用して限定されます。データベース構成の範囲は、コマンドレットによってメールボックスを移動できる移動先のデータベースを制御します。メールボックスを移動するデータベースは、データベースの範囲内にある必要があります。この条件は、**New-MoveRequest** コマンドレットで *TargetDatabase* パラメーターを使用してデータベースを指定する場合、または自動メールボックス配信によるデータベースの選択を許可する場合に適用されます。詳細については、「[管理役割スコープについて](understanding-management-role-scopes-exchange-2013-help.md)」を参照してください。
