---
title: "\"メール受信者の作成\" 役割: Exchange 2013 Help"
TOCTitle: "\"メール受信者の作成\" 役割"
ms:assetid: 81081845-9ebf-4888-8f9f-fe7cf2704486
ms:mtpsurl: https://technet.microsoft.com/ja-jp/library/Dd876915(v=EXCHG.150)
ms:contentKeyID: 49896339
ms.date: 04/24/2018
...
この役割を使用しても、パブリック フォルダーのメールを有効にすることはできません。パブリック フォルダーのメールを有効にするには、`Mail Enabled Public Folders` 役割を使用する必要があります。詳細については、「["Mail Enabled Public Folders/メールが有効なパブリック フォルダー" 役割](mail-enabled-public-folders-role-exchange-2013-help.md)」を参照してください。

受信者を管理するグループとは異なるグループが受信者を作成する、役割ベースのアクセス制御 (RBAC) 分割型アクセス許可モデルを組織が維持している場合は、受信者の作成を実行する管理役割グループに `Mail Recipient Creation` 役割を割り当て、受信者の管理を実行する役割グループに `Mail Recipients` 役割を割り当てます。

組織が Active Directory 分割型アクセス許可を有効にしている場合、この管理役割に対して委任されていない管理役割の割り当てはすべて削除されています。Active Directory 分割型アクセス許可が有効になっている場合、Active Directory 管理者のみが Active Directory 管理ツールを使用して、ユーザーやセキュリティ グループなどの新しいセキュリティ プリンシパルを作成できます。

RBAC と Active Directory 分割型アクセス許可の詳細については、「[分割型アクセス許可について](understanding-split-permissions-exchange-2013-help.md)」を参照してください。
...
この役割の対象に含まれる **New-Mailbox** コマンドレットには、受信者スコープに加え、データベース構成スコープも適用されます。データベース構成スコープは、このコマンドレットで新しいメールボックスを作成できるデータベースを制御します。メールボックスの作成先のデータベースは、データベース スコープ内にある必要があります。この条件は、**New-Mailbox** コマンドレットで *Database* パラメーターを使用してデータベースを指定する際や、自動メールボックス配布によって代わりにデータベースを選択する場合に適用されます。詳細については、「[管理役割スコープについて](understanding-management-role-scopes-exchange-2013-help.md)」を参照してください。
